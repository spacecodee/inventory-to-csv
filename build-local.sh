...
echo "$OPENROUTER_API_KEY" > /tmp/openrouter_api_key
...
DOCKER_BUILDKIT=1 docker build -t inventory-to-csv:local \
  ...
  --secret id=openrouter_api_key,src=/tmp/openrouter_api_key \
  --secret id=supabase_url,src=/tmp/supabase_url \
  --secret id=supabase_anon_key,src=/tmp/supabase_anon_key \
  .

rm /tmp/google_gemini_api_key /tmp/openrouter_api_key /tmp/supabase_url /tmp/supabase_anon_key
